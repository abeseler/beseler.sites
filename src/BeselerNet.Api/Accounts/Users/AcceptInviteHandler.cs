...
internal sealed class AcceptInviteHandler
{
        ...
        CancellationToken cancellationToken)
    {
        if (!int.TryParse(principal.FindFirstValue(JwtRegisteredClaimNames.Sub), out var accountId)
            ...
            || principal.FindFirstValue(InviteUserHandler.InviteClaim) is null)
        {
        ...
        var problem = account switch
        {
            ...
            { Email: var stored } when stored != email => new()
            {
                Title = "Invalid Invite",
                Detail = "The invite link is invalid.",
                Status = StatusCodes.Status403Forbidden
            ...
            { IsInvited: false } => AccountProblems.NotInvited,
            _ => null
        };
        ...
        var hash = passwordHasher.HashPassword(account!, request.Password!);
        account!.AcceptInvite(hash);
        await accounts.SaveChanges(account, cancellationToken);

        account = await accounts.WithId_IncludePermissions(accountId, cancellationToken) ?? account;
